...
import { canvas, canvas2d } from './2DCanvas';
...
export default class Draw2D {
	...
	public static drawHealthBar2D(x: number, y: number, currentHp: number, maxHp: number = 10) {
		...
		canvas2d.fillRect(x - barWidth / 2, y, barWidth, barHeight);

		// fill
		canvas2d.fillStyle = 'green';
		canvas2d.fillRect(x - barWidth / 2, y, barWidth * progress, barHeight);
	}
	...
	public static drawModal(world: World): void {
		...
		const modalHeight = modalOptions.length * 15;
		...
		modalOptions.forEach((option, index) => {
			...
			const optionY = modalY + 2 + index * 14;
